            ...
            if dyn_objects in data["dynobj"][ex]["object_name"]:
                ...
                if (dignew_set != digold_set):
                    ...
                    if flag == 0:
                        ...
                        ssh_client.connect(hostname=fw_data["firewalls"][v]["fw_ip_address"], port=22, username='admin',
                                           password=fw_data["firewalls"][v]["fw_password"], look_for_keys=False,
                                           allow_agent=False)
                        flag = 1
                    ...
                    if len(ip_add) != 0:
                        print("Dynmic Object =", data["dynobj"][ex]["object_name"], " firewall = ",
                              fw_data["firewalls"][v]["fw_hostname"])
                        for ax in ip_add:
                            print("IP to be added", ax)
                            shell.send('dynamic_objects -o ' + data["dynobj"][ex][
                                "object_name"] + ' -r ' + ax + ' ' + ax + ' -a\n')
                            time.sleep(1)
                            print('dynamic_objects -o ' + data["dynobj"][ex][
                                "object_name"] + ' -r ' + ax + ' ' + ax + ' -a\n')
                            output = shell.recv(10000)
                            output = output.decode('utf-8')
                            print(output)
...
for v in u:
    th = threading.Thread(target=dynamic_DNS_update, args=(v))
    threads.append(th)
